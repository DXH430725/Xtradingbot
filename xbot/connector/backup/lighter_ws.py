...
class LighterWsClient:
    ...
    - Public: order_book/{market_index}, trade/{market_index} (best-effort)
    - Private: account_orders/{market_index}/{account_index} with auth token
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Simple in-memory order book for best price calculation
    ...
    async def _build_auth_token(self) -> Optional[str]:
        try:
            ...
            if not priv:
                self._logger.info("ws_auth_missing_priv", extra={"venue": "lighter", "key_file": str(self._key_file) if self._key_file else None})
                return None
            import lighter  # type: ignore
            acct_idx = int(_kg("account_index", "accountindex") or 0)
            key_idx = int(_kg("api_key_index", "apikeyindex") or 0)
            self._logger.info("ws_auth_params", extra={"venue": "lighter", "account_index": acct_idx, "api_key_index": key_idx})
            ...
            # Proactively close the signer's api client to avoid session leaks
            try:
            ...
            # Attempt to close default client if created
            try:
                import lighter  # type: ignore
    ...
    async def _subscribe(self, ws, private: bool, auth_token: Optional[str]) -> None:
        # Public order book
        await ws.send(json.dumps({"type": "subscribe", "channel": f"order_book/{self._market_index}"}))
        # Public trades (best-effort)
        with contextlib.suppress(Exception):
            await ws.send(json.dumps({"type": "subscribe", "channel": f"trade/{self._market_index}"}))
        # Private account orders (only with auth)
        if private and self._account_index is not None and auth_token:
        ...
        while self._running.is_set():
            ...
            try:
                self._logger.info(
                    ...
                    extra={
                        ...
                        "streams": [
                            ...
                            (f"account_orders/{self._market_index}/{self._account_index}" if (auth_token and self._account_index is not None) else None),
                        ],
                ...
                async with websockets.connect(
                    ...
                    self._logger.info("ws_connected", extra={"venue": "lighter", "has_private": has_private})
                    # Reset book on new connection
                    self._bids.clear(); self._asks.clear(); self._ob_offset = None
                    async for raw in ws:
                        try:
                            msg = json.loads(raw)
                        except Exception:
    ...
    async def _handle_message(self, msg: Dict[str, Any]) -> None:
        ...
        try:
            if et == "subscribed/order_book":
                ...
                # Handle offset progression robustly:
                # - If new_offset is missing, apply deltas conservatively.
                # - If duplicate offset (==), ignore to avoid double-apply.
                # - If exactly +1, apply.
                # - If > +1, treat as gap and trigger reconnect.
                if isinstance(self._ob_offset, int) and isinstance(new_offset, int):
                    ...
                    if new_offset > self._ob_offset + 1:
                        raise RuntimeError(f"order_book offset gap: have {self._ob_offset}, got {new_offset}")
                # Apply updates and advance offset
                self._apply_ob_updates("bids", ob.get("bids") or [])
                self._apply_ob_updates("asks", ob.get("asks") or [])
                ...
                # Some servers send batch trades under key 'trades'
                if isinstance(msg.get("trades"), list):
                    ...
                    await self._cache.add_trade(self._venue_symbol, norm)
            elif et in ("subscribed/account_orders", "update/account_orders"):
    ...
    async def _handle_account_msg(self, msg: Dict[str, Any]) -> None:
        # Lighter account_orders stream: expect `order_updates` list
        orders = msg.get("order_updates")
        parsed_any = False
        if isinstance(orders, list):
            for item in orders:
                if isinstance(item, dict):
                    ...
                    parsed_any = True
        if not parsed_any:
            self._logger.info("ws_account_no_orders", extra={"venue": "lighter", "keys": list(msg.keys())[:10]})

        # Optionally capture positions if provided alongside updates
        positions = (
            msg.get("positions")
            or (msg.get("data") or {}).get("positions")
            or (msg.get("account") or {}).get("positions")
        )
    ...
    async def _ingest_order_update(self, data: Dict[str, Any]) -> None:
        if not self._on_order_update:
            return
        try:
            coi = data.get("client_order_index") or data.get("client_order_id") or data.get("coi") or data.get("clientId")
            if coi is None:
                return
            ...
            # Default state from mapping
            state = mapping.get(state_raw)
            # Hyphenated cancel reasons like canceled-expired
            if state is None and state_raw.startswith("canceled"):
                state = OrderState.CANCELLED
            # Derive partial/filled from amounts when status is still 'open' or 'in-progress'
            ...
            exchange_order_id = str(data.get("order_index") or data.get("orderId") or data.get("i") or "")
