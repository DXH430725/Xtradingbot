...
# Do not configure global logging here; leave it to applications.
...
def _initialize_signer():
    ...
    if is_arm and is_mac:
        ...
        raise Exception(
            f"Unsupported platform/architecture: {sys_name}/{platform.machine()} only supports Linux(x86_64), Windows(x86_64) and Darwin(arm64)"
        )
...
def process_api_key_and_nonce(func):
    ...
    async def wrapper(self, *args, **kwargs):
        ...
        try:
            ...
            # If the call returned an error or no response, acknowledge failure and propagate.
            if err is not None or ret is None:
                self.nonce_manager.acknowledge_failure(api_key_index)
                return created_tx, ret, err
            # Non-OK codes should also be treated as failures for nonce tracking
            if getattr(ret, "code", None) != CODE_OK:
                self.nonce_manager.acknowledge_failure(api_key_index)
