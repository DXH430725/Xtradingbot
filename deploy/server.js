...
// 清理离线机器人：POST /admin/prune?older_ms=0
app.post('/admin/prune', (req, res) => {
  if (AUTH_TOKEN && req.get('x-auth-token') !== AUTH_TOKEN) {
    return res.status(401).json({ status: 'unauthorized' });
  }

  const olderMs = Number(req.query.older_ms || 0);
  const now = Date.now();
  let removed = 0;

  for (const [name, bot] of bots) {
    const intervalSec = Number(bot.payload?.telemetry_interval_secs || 10);
    const offline = now - bot.ts >= intervalSec * 2000;
    const tooOld = olderMs > 0 && now - bot.ts >= olderMs;
    if (offline || tooOld) {
      bots.delete(name);
      removed++;
    }
  ...
  res.json({ status: 'ok', removed, total: bots.size });
});
