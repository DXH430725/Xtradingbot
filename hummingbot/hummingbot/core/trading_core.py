...
class TradingCore:
    ...
    def __init__(self,
        ...
        Args:
            ...
            debug: Optional debug switch to enable verbose debug logs
        """
        ...
        self.debug_enabled: bool = self._resolve_debug_flag(debug)

    def _resolve_debug_flag(self, arg_value: Optional[bool]) -> bool:
        """Resolve debug flag from arg, config, or env (XTB_DEBUG)."""
        if arg_value is not None:
            return bool(arg_value)
        try:
            cfg_val = getattr(self.client_config_map, "debug", None)
    ...
    def dbg(self, msg: str, exc_info: bool = False):
        """Emit debug logs only when enabled."""
    ...
    async def start_clock(self) -> bool:
        ...
        if self.clock is not None:
            ...
            self.dbg("start_clock called but clock already running")
            return False

        try:
            tick_size = getattr(self.client_config_map, "tick_size", 1.0)
            self.logger().info(f"Creating the clock with tick size: {tick_size}")
            self.dbg(f"Clock init: mode=REALTIME, tick_size={tick_size}")
            ...
            for connector in self.connector_manager.connectors.values():
                if connector is not None:
                    ...
                    self.dbg(f"Added connector to clock: {getattr(connector, 'name', 'unknown')}")
            ...
            self.dbg("Clock task started")
            return True

        except Exception as e:
            self.logger().error(f"Failed to start clock: {e}")
            self.dbg("start_clock exception", exc_info=True)
            return False
    ...
    async def stop_clock(self) -> bool:
        ...
        try:
            ...
            self.dbg("Clock stopped and cleared")
            return True

        except Exception as e:
            self.logger().error(f"Failed to stop clock: {e}")
            self.dbg("stop_clock exception", exc_info=True)
            return False
    ...
    async def create_connector(self,
        ...
        """
        self.dbg(f"create_connector: name={connector_name}, pairs={trading_pairs}, trading_required={trading_required}")
        ...
        if self.clock and connector:
            self.clock.add_iterator(connector)
            self.dbg(f"Connector added to running clock: {connector_name}")

        # Add to markets recorder if exists
        if self.markets_recorder and connector:
            self.markets_recorder.add_market(connector)
            self.dbg(f"Connector added to markets recorder: {connector_name}")
    ...
    def is_script_strategy(self, strategy_name: str) -> bool:
        ...
        self.dbg(f"is_script_strategy({strategy_name}) -> {exists}")
        return exists
    ...
    def initialize_markets_recorder(self, db_name: str = None):
        ...
        self.markets_recorder = MarketsRecorder(
            ...
            getattr(self.client_config_map, "market_data_collection", False)
        )

        self.markets_recorder.start()
        self.logger().info(f"Markets recorder initialized with database: {db_name}")
        self.dbg(f"Markets recorder started (db={db_name})")
    ...
    async def start_strategy(self,
        ...
        try:
            if self._strategy_running:
                ...
                self.dbg("start_strategy called while a strategy is running")
                return False
            ...
            self.dbg(f"start_strategy: detected type {strategy_type} for '{strategy_name}'")
            ...
            self.dbg("Strategy started and execution loop initialized")
            return True

        except Exception as e:
            self.logger().error(f"Failed to start strategy {strategy_name}: {e}")
            self.dbg("start_strategy encountered exception", exc_info=True)
            return False
    ...
    async def _initialize_script_strategy(self):
        ...
        self.dbg("Script strategy instance created")
    ...
    async def _initialize_regular_strategy(self):
        ...
        self.dbg("Regular strategy starter executed")
    ...
    async def _start_strategy_execution(self):
        ...
        try:
            ...
            if self.strategy and self.clock:
                ...
                self.dbg("Strategy added to clock")
            ...
            self.dbg("_start_strategy_execution exception", exc_info=True)
            raise
    ...
    async def stop_strategy(self) -> bool:
        ...
        try:
            ...
            self.dbg("Strategy stopped and cleaned up")
            return True

        except Exception as e:
            self.logger().error(f"Failed to stop strategy: {e}")
            self.dbg("stop_strategy exception", exc_info=True)
            return False
    ...
    async def cancel_outstanding_orders(self) -> bool:
        ...
        try:
            ...
            self.dbg("cancel_outstanding_orders exception", exc_info=True)
            return False
    ...
    async def shutdown(self, skip_order_cancellation: bool = False) -> bool:
        ...
        try:
            ...
            self.dbg("Shutdown completed; state reset")
            return True

        except Exception as e:
            self.logger().error(f"Error during shutdown: {e}")
            self.dbg("shutdown exception", exc_info=True)
            return False
